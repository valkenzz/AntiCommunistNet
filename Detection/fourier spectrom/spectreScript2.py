...
number_iter = 4999
#fake data
rootdir = '/media/seagate/vmeo/detectionDataSet/SnGan/route/F/img/'
#real data
rootdir2 = '/media/seagate/vmeo/detectionDataSet/SnGan/route/T/'

saveIMG='./TTTTTTTTTTTTTTT.png'
...
#            print(mask)
        ...
        def crop(image):
            ...
            if np.max(y_nonzero)-np.min(y_nonzero)!=130:
                taille=(np.max(y_nonzero)-np.min(y_nonzero))
                if taille<130:
                        if (np.max(y_nonzero)+(130-taille))>255:
                           minY=minY-(130-taille) 
                        else:
                            maxY=maxY+(130-taille)
                            
                else:        
                   maxY=minY+130
            if np.max(x_nonzero)-np.min(x_nonzero)!=130: 
                 taille=(np.max(x_nonzero)-np.min(x_nonzero))
                 if taille<130:
                         if (np.max(x_nonzero)+(130-taille))>255:
                            minx=minx-(130-taille) 
                         else:
                             maxx=maxx+(130-taille)
