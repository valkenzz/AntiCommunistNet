...
def trainNet(net, data_dir, sample_dir, cpt_dir, epochs=100, gpu=True, train=True, pth=None):
    ...
    if train:
        #train_dataset = dataset.InpaintingDataSet(os.path.join(data_dir, '33978.tif_7_4.png'), 1600)
        train_dataset = dataset.InpaintingDataSet(data_dir, 1600)
       ...
       # test_dataset = dataset.InpaintingDataSet(os.path.join(data_dir, 'test.png'), 5, train=False)
        test_dataset = dataset.InpaintingDataSet(data_dir, 5, train=False)
...
def getArgs():
    ...
    parser.add_option('--data-dir', dest='data_dir', default='/media/seagate/vmeo/residencielle/petit', help='data directory')
