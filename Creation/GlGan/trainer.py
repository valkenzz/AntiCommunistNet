...
class Trainer(nn.Module):
    ...
    def save_model(self, checkpoint_dir, iteration):
        ...
        torch.save(self.netG, str( "./g"+ str(iteration)))
    ...
    def resume(self, checkpoint_dir, iteration=0, test=False):
        ...
        self.netG.load_state_dict(torch.load(str("./checkpoints/imagenet/hole_benchmark/gen_00180000.pt")))
        torch.save(self.netG, "./g")
        iteration = 18000 #int(last_model_name[-11:-3])

        if not test:
            # Load discriminators
            #last_model_name = get_model_list(checkpoint_dir, "dis", iteration=iteration)
            #state_dict = torch.load(last_model_name)
            state_dict = torch.load(str("./checkpoints/imagenet/hole_benchmark/dis_00230000.pt"))
            self.localD.load_state_dict(state_dict['localD'])
            self.globalD.load_state_dict(state_dict['globalD'])
            # Load optimizers
            #state_dict = torch.load(os.path.join(checkpoint_dir, 'optimizer.pt'))
            state_dict = torch.load("/home/vmeo/GLGan/checkpoints/imagenet/hole_benchmark/optimizer.pt")
